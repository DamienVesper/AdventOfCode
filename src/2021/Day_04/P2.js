...
class Bingo {
    ...
    callNum = num => {
        ...
        for (const board of [...this.boards.values()]) {
            board.apply(num);
            if (board.bingo && !this.winners.includes(board)) this.winners.push(board);
        }

        if (this.winners.length === this.boards.length) {
    ...
    calculateWinner = () => {
        const board = this.winners[this.winners.length - 1];
